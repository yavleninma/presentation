...
Пользователь начинает с короткого рабочего запроса, сразу получает шестислайдовый draft, уточняет его через чат и только потом переходит в editor для локальной пересборки активного слайда.
...
- Нет экспорта.
- Нет расширенного набора стартовых сценариев поверх текущего демо-потока.
...
`npm run smoke` сам собирает production build, поднимает локальный сервер, мокает `/api/draft` и проверяет путь `start -> draft -> editor` без реального `OPENAI_API_KEY`.
