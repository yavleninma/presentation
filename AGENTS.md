...
Агент может по своему усмотрению запускать сабагентов, когда от параллельной или разделённой работы есть явная польза для полезного результата; отдельное согласование пользователя не требуется.
...
Тексты из `VNYATNO.md`, `MUTNO.md`, `DESIGN_SPINE.md`, `AGENTS.md`, `AGENTS_BRAND_KIT_sovcom-inspired.md` — это инструкции для агента.
Они **никогда** не должны попадать в интерфейс. Ни целиком, ни в пересказе.

Примеры формулировок из `AGENTS_BRAND_KIT_sovcom-inspired.md` не переносить в продуктовый UI как готовый копирайт: сначала рабочий сценарий и проверка по `COPY_RULES.md`.
...
Основной Figma-файл: `CXBGVOHmhpond41SwSwxTf`
Ссылка: https://www.figma.com/design/CXBGVOHmhpond41SwSwxTf

Актуальные страницы и их роли описаны в `DESIGN_DECISIONS.md`.

---

## Дизайн-система и опора для UI

HTML-референс `references/redesign-2026-04/preview.html` **в репозитории отсутствует** (каталог `references/` снят). На него не опираться.

Порядок чтения для интерфейса прототипа в `presentations-frontend`:

1. **`DESIGN_DECISIONS.md`** — раздел **«v3 — Brand Kit (актуальная пересборка)»** и ссылка на Figma-страницу v3.
2. **Figma** — файл `CXBGVOHmhpond41SwSwxTf`, страница **v3 — Brand Kit** (роли экранов и визуал).
3. **`DESIGN_SPINE.md`**, **`UI_KIT_V0.md`**, **`SCREEN_ATLAS.md`**, **`COPY_RULES.md`** — маршрут, вайрфреймы, грамматика UI, тексты.

Если документы спорят друг с другом, до их переписывания: по **визуалу, токенам и хрому** (отступы, радиусы, палитра, типографика) приоритет у **v3 в Figma и в `DESIGN_DECISIONS.md`**; по **маршруту, ролям экранов и скрытым проходам** (в т.ч. `fit-pass` не как отдельный экран) — у согласованного слоя **spine + atlas + фактическое поведение кода прототипа**.

Не придумывай новые экраны, режимы, компоненты, паттерны навигации или декоративные решения врозь с этой опорой. Точечные изменения сопровождай явным конфликтом и предложением, какой документ обновить, прежде чем менять код.

**Сначала дизайн-основание, потом код. Не наоборот.**

## Агентский бренд-кит (опционально)

В корне репозитория лежит `AGENTS_BRAND_KIT_sovcom-inspired.md`: принципы подачи «выгода → число → простое объяснение → действие», светлая сцена и насыщенные акценты, ориентиры для промо и иллюстраций. В самом файле зафиксировано: это **не** официальный чужой брендбук и не копия айдентики.

Когда открывать:

- задачи про лендинги, промо-страницы, витринные макеты, брифы на визуал или внешние коммуникации, если это уместно по задаче пользователя;
- когда пользователь явно просит близкую к описанному стилю подачу.

Когда **не** подменяет остальное:

- маршрут, экраны, компоненты и пользовательский копирайт в продукте опираются на **v3 (Figma + `DESIGN_DECISIONS.md`)**, `DESIGN_SPINE.md`, `UI_KIT_V0.md`, `SCREEN_ATLAS.md` и `COPY_RULES.md`;
- при противоречии приоритет у этой продуктовой опоры, а не у бренд-кита.

## Где лежат старые правила Cursor

Правила `.mdc` для архивного SlideForge остались в `old/.cursor/rules/`. К текущему приложению в `presentations-frontend/` они **не** привязаны; не использовать их как источник истины для нового продукта, если задача явно не про каталог `old/`.
